...
const TEXTS = {
  emailTitle: '사용할 이메일을 입력해주세요',
  emailDescription: `로그인에 사용할 이메일을 입력해주세요.\n인증 번호를 보내드립니다.`,
...
export const SignupAuthScreen: React.FC = () => {
  ...
  const handleEmailSubmit = async (data: EmailFormData) => {
    try {
      setIsLoading(true);
      setError(null);
      await authApi.requestSignup(data.email);
      setEmail(data.email);
  ...
  const handleCodeSubmit = useCallback(
    async (data: VerifyCodeFormData) => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        const response = await authApi.verifySignup(email, data.code);
