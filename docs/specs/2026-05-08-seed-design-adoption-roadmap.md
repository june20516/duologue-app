...
> **Phase 1-A에서 이관된 작업이 있음.** [Phase 1-A spec § 5](2026-05-13-phase-1a-semantic-tokens.md#5-후속-작업-deferred--반드시-다음-단계에서-처리)에 컴포넌트의 원시 토큰 직참조 제거, Toast의 `$*Light` alias 제거 항목이 명시되어 있다. 아래 2-B(Form)·2-D(Toast) sub-project 시작 시 해당 § 5 항목들이 작업 범위에 포함됐는지 체크하고, `docs/tasks/pending/phase2-b-form-semantic-migration.md` / `phase2-d-toast-semantic-cleanup.md` 문서를 입력으로 사용한다.
