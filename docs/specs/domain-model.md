...
- **API 클라이언트**: ConnectRPC (@connectrpc/connect-web)
- **UI 라이브러리**: Tamagui
...
**API 서비스 메서드** (ConnectRPC):
- `authServiceClient.requestSignup()` - 회원가입 인증 코드 요청
- `authServiceClient.verifySignup()` - 인증 코드 검증 및 회원가입
- `authServiceClient.requestLogin()` - 로그인 코드 요청
- `authServiceClient.verifyLogin()` - 로그인 코드 검증
- `authServiceClient.refresh()` - 토큰 갱신
- `authServiceClient.logout()` - 로그아웃
- `authServiceClient.me()` - 현재 사용자 정보

**화면 플로우**:
1. `/auth/signin` 또는 `/auth/signup` - 이메일 입력 및 인증 코드 입력
2. `/onboarding` (회원가입 후) - 프로필 설정
3. `/(tabs)` - 메인 화면
...
**API 서비스 메서드** (ConnectRPC):
- `profileServiceClient.me()` - 내 프로필 조회
- `profileServiceClient.update()` - 프로필 수정
- `profileServiceClient.get()` - 타인 프로필 조회 (기본 정보)
- `profileServiceClient.get({ level: 'intimate' })` - 상세 프로필 조회 (친밀함 상태)
...
├── walkthrough.tsx                # 최초 실행 안내 화면
│
├── auth/                          # 인증 플로우
│   ├── _layout.tsx               # 인증 레이아웃
│   ├── index.tsx                 # 인증 홈
│   ├── signin.tsx                # 로그인 (이메일 + 코드 입력)
│   └── signup.tsx                # 회원가입 (이메일 + 코드 입력)
│
├── onboarding/                    # 온보딩 (프로필 설정)
│   ├── _layout.tsx               # 온보딩 레이아웃
│   ├── index.tsx                 # 온보딩 진입점
│   └── _components/              # 온보딩 컴포넌트
│       ├── ProfileFlowLayout.tsx # 프로필 플로우 레이아웃
│       ├── ProfileStep.tsx       # 프로필 입력 스텝
│       ├── InterestStep.tsx      # 관심사 선택 스텝
│       └── OptionalStep.tsx      # 선택사항 스텝
│
├── (tabs)/                        # 메인 탭 네비게이션
│   ├── _layout.tsx               # 탭 레이아웃
│   ├── index.tsx                 # 홈 (매칭 히스토리)
...
import { AuthService } from '@/proto/duologue/v1/auth_pb';
...
const transport = createConnectTransport({
  ...
  interceptors: [
    ...
    // Logging 인터셉터
    (next) => async (req) => {
      console.log(`[API] ${req.method.name}`);
      const response = await next(req);
      console.log(`[API] ${req.method.name} - ${response.status}`);
      return response;
    },
  ],
});
...
export const authApi = {
  ...
  requestSignup: async (email: string) => {
    const response = await authServiceClient.requestSignup({ email });
    return toCamelCase(response);
  },

  // 회원가입 검증
  verifySignup: async (email: string, code: string) => {
    const response = await authServiceClient.verifySignup({ email, code });
    return toCamelCase(response);
  },

  // 로그인 요청
  requestLogin: async (email: string) => {
    const response = await authServiceClient.requestLogin({ email });
    return toCamelCase(response);
  },

  // 로그인 검증
  verifyLogin: async (email: string, code: string) => {
    const response = await authServiceClient.verifyLogin({ email, code });
    return toCamelCase(response);
  },

  // 토큰 갱신
  refresh: async (refreshToken: string) => {
    const response = await authServiceClient.refresh({ refreshToken });
    return toCamelCase(response);
  },
...
**토큰 갱신 로직**:
- 자세한 내용은 [token-management.md](./token-management.md) 참조
- `api/tokenRefresh.ts`의 `TokenRefreshManager` 클래스가 중복 갱신 방지
