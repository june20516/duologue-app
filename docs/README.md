# duologue-server 문서 관리 가이드

이 문서는 duologue-server 프로젝트의 문서 기반 개발 프로세스를 설명합니다.
...
│   ├── adr/          # Architecture Decision Records (설계 결정 기록)
...
- 하이픈(-)으로 단어 구분
- 명확하고 간결한 이름
- 예: `user-crud-api.md`, `rate-limiting.md`
...
```bash
cp docs/templates/spec.md docs/specs/user-api.md
...
```bash
cp docs/templates/task.md docs/tasks/pending/user-crud-api.md
...
```bash
mv docs/tasks/pending/user-crud-api.md docs/tasks/active/
```

**4단계: Claude에게 지시**

```
docs/tasks/active/user-crud-api.md를 읽고,
...
```bash
# 문서에 완료 내역 기록
mv docs/tasks/active/user-crud-api.md docs/tasks/completed/
```
...
- `docs/architecture/adr/`: 아키텍처 설계 결정 기록
- `docs/architecture/standards/`: 코딩 표준 및 규칙
